...
def _filtergraph(framing, sub_name, dur=0.0, grade_film=False, grain=False, ratio="9:16"):
    ...
    grade = f",{cinema.cinematic_filters(grain=grain)}" if grade_film else ""
    W, H = cinema.ratio_size(ratio)
    if framing == "crop-left":
        return None, f"crop=ih*9/16:ih:0:0,scale={W}:{H},{fv_in},{fv_out}{grade}{sub}"
    if framing == "crop-right":
        return None, f"crop=ih*9/16:ih:(iw-ih*9/16):0,scale={W}:{H},{fv_in},{fv_out}{grade}{sub}"
    if framing == "crop":
        return None, f"crop=ih*9/16:ih:(iw-ih*9/16)/2:0,scale={W}:{H},{fv_in},{fv_out}{grade}{sub}"
    # blur-pad: full frame visible, background = zoomed blurred copy
    chain = ("[0:v]split=2[bg][fg];"
             f"[bg]scale={W}:{H}:force_original_aspect_ratio=increase,"
             f"crop={W}:{H},boxblur=20:5[bgb];"
             "[fg]scale=%d:-2[fgs];" % W +
             f"[bgb][fgs]overlay=(W-w)/2:(H-h)/2,format=yuv420p,{fv_in},{fv_out}{grade}{sub}[v]")
    return chain, None


def _audio_fade(dur, lead=0.0):
    """Audio fade in/out. `lead` > 0 adds a J-cut style audio lead: the audio starts slightly
    before the picture would, so the incoming cut carries the viewer in."""
    return (f"afade=t=in:st=0:d=0.15,"
            f"afade=t=out:st={max(0.0, dur - 0.18):.2f}:d=0.18")


def cut_clip(src, start, end, out, sub_name=None, cwd=None, framing="blur",
             grade=True, grain=False, ratio="9:16", audio_lead=0.0):
    ...
    complex_fg, simple_vf = _filtergraph(framing, sub_name, duration,
                                         grade_film=grade, grain=grain, ratio=ratio)
    lead = max(0.0, float(audio_lead or 0.0))
    af = ["-af", _audio_fade(duration)]
    if lead > 0.01:
        # pull the picture a touch later so the audio (which starts at -ss start-lead) is already
        # running when the picture appears — the incoming cut "leads" with sound
        af = ["-af", f"adelay={int(lead * 1000)}|{int(lead * 1000)}," + _audio_fade(duration + lead)]
    base = [ffmpeg(), "-y", "-ss", f"{max(0.0, start - lead):.2f}", "-i", src,
            "-t", f"{(duration + lead):.2f}"]
