...
from . import captions, cinema, ffmpeg_tools, social, stt, virality
...
def _render_clip(job, src_path, m, base, clips_dir):
    ...
    note = ffmpeg_tools.cut_clip(src_path, m["start"], m["end"],
                                 ...
                                 grade=job.get("cinematic_grade", True),
                                 grain=job.get("cinematic_grain", False),
                                 ratio=job.get("ratio", "9:16"),
                                 audio_lead=job.get("audio_lead", 0.35))
    return {
...
def process(job_id, src_path):
    ...
    try:
        ...
        _set(job_, mode=f"{mode}+{picker}", content_type=content_type, picker=picker,
             engine="B2 Pro X", shots=len(cinema_marks),
