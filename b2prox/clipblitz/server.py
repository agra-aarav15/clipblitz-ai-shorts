"""B2 Pro X web server — stdlib ThreadingHTTPServer on :4302.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/health":
            ...
            return self._json(200, {
                "ok": True, "name": "B2 Pro X", "version": "1.0.0-b2",
                "engine": "B2 Pro X", "ffmpeg": ffmpeg_available(), "stt": stt_mode(),
    ...
    def _success_page(self, msg, ok=True):
        ...
        return ("<!doctype html><html><head><meta charset='utf-8'>"
                ...
                "<small>returning to B2 Pro X</small></div></body></html>").encode("utf-8")
...
def serve(port=None):
    ...
    try:
        ...
        print(f"B2 Pro X is already running → http://localhost:{port}  (open that tab; "
              f"or kill the old instance first)")
        sys.exit(0)
    print(f"B2 Pro X (cinematic engine) → http://localhost:{port}  "
