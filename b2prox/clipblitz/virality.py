...
def rank(segments, duration, count=3, energy=None, laughs=None, scenes=None,
    ...
    for c in cands:
        snap(c, segments, duration)
    # ---- B2 CINEMA PASS (deterministic) ------------------------------------------
    # 1. snap edges onto real shot boundaries so cuts land like an edit, not an arbitrary stop
    # 2. re-time to the scene grammar: open on the establishing beat, land the payoff on the
    #    peak shot (highest measured on-screen motion inside the cut)
    if cinema_marks:
        from . import cinema as _cin
        for c in cands:
            if not isinstance(c, dict):
                continue
            a, b = _cin.cinema_snap(c["start"], c["end"], cinema_marks, segments, duration)
            if a != c["start"] or b != c["end"]:
                c["start"], c["end"] = a, b
                c["cinema"] = "cut on the shot boundary"
            if motion:
                a2, b2, note = _cin.scene_grammar(c["start"], c["end"], motion, cinema_marks, segments)
                if a2 != c["start"] or b2 != c["end"]:
                    c["start"], c["end"] = a2, b2
                    c["cinema"] = (c.get("cinema", "") + ("; " if c.get("cinema") else "") + note)
