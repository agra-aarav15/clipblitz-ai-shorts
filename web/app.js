...
function fileChosen() {
  ...
  if (!f) return;
  dz.querySelector('.big').innerHTML = `${IC('tape')} ${esc(f.name)} (${(f.size / 1048576).toFixed(0)} MB)`;
...
function clipCardHTML(job, c, i) {
  ...
  return `
    ...
    <div class="reason">${IC('bulb')} ${esc(c.reason)}</div>
    <div class="clipmeta">
    ...
    <div class="metabox">
      ...
      <div class="row">
        <button class="btn ghost small" data-save="${i}">${IC('save')} Save metadata</button>
...
function postChips(clip, i) {
  return PLATFORMS.map(p => {
    ...
    if (st?.status === 'published') { cls = 'done'; label = `${IC('check')} ${PLATFORM_LABEL[p]} published`; }
    if (st?.status === 'assisted_ready') { cls = 'done'; label = `${IC('list')} ${PLATFORM_LABEL[p]} ready — caption copied`; }
    if (st?.status === 'error') { cls = 'err'; label = `${IC('warn')} ${PLATFORM_LABEL[p]}: ${st.note || 'failed'}`; }
...
function buildLab(job) {
  ...
  grid.innerHTML = cands.map((c) => `
    <div class="labcards glass" data-cand="${job.candidates.indexOf(c)}">
      ...
      <button class="btn ghost small" data-render="${job.candidates.indexOf(c)}">${IC('bolt')} Render this clip</button>
    </div>`).join('');
  grid.querySelectorAll('[data-render]').forEach(b => b.addEventListener('click', async () => {
    ...
    try {
      ...
      b.disabled = false; b.innerHTML = `${IC('bolt')} Render this clip`;
...
async function refreshSocial() {
  try {
    ...
    if (yt.connected) {
      ...
      $('yt-status').innerHTML = IC('warn') + ' OAuth not set up yet';
...
(async function latestJobResume() {
  try {
    ...
    btn.innerHTML = `${IC('strip')} Show the latest clips (${esc(latest.name || latest.id)})`;
